  ...
  return (
    <div className="slide-up">
      ...
      <div style={{ background: C.white, borderRadius: 16, padding: 14, marginBottom: 12, display: 'flex', gap: 12, alignItems: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
        {preview && <img src={preview} alt="Scanned" style={{ width: 58, height: 58, borderRadius: 10, objectFit: 'cover', border: `1px solid ${C.gray100}`, flexShrink: 0 }} />}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 800, fontSize: 17, color: C.navy, lineHeight: 1.2, marginBottom: 2 }}>{results.brandName || 'Medicine'}</div>
      ...
      <div style={{ background: '#FEF3C7', border: '1px solid #FCD34D', borderRadius: 12, padding: '8px 12px', marginBottom: 12, fontSize: 12, color: '#92400E', lineHeight: 1.5 }}>
        ⚠️ <strong>Beta:</strong> AI-estimated. Full CDSCO database integration coming soon. Verify with a pharmacist.
      </div>
      ...
      <p style={{ fontSize: 11, color: C.gray500, textAlign: 'center', lineHeight: 1.6, margin: '16px 0 12px' }}>
        For awareness only — not medical advice. Always consult a doctor or pharmacist.
      </p>

      <button onClick={onReset} style={{ width: '100%', background: C.green, color: '#fff', padding: '16px', borderRadius: 16, fontSize: 16, fontWeight: 700, boxShadow: '0 4px 14px rgba(15,122,90,0.3)' }}>
...
function AuthenticityCard({ auth }) {
  ...
  const configs = {
    ...
    LIKELY_FAKE:      { bg: '#FEF2F2', border: '#FCA5A5', icon: '🚨', color: '#DC2626', headline: 'Possible Fake — Be Careful' },
    CANNOT_DETERMINE: { bg: '#FFFBEB', border: '#FCD34D', icon: '🔍', color: '#D97706', headline: 'Cannot Determine' },
  }
    ...
    <div style={{ background: cfg.bg, border: `2px solid ${cfg.border}`, borderRadius: 18, marginBottom: 12, overflow: 'hidden' }}>
      <div style={{ padding: '14px 14px 10px' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.gray500, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 6 }}>
          REAL OR FAKE?
          <span style={{ marginLeft: 8, background: C.navy, color: '#fff', borderRadius: 6, padding: '2px 7px', fontSize: 10 }}>AI Estimated</span>
        </div>
      ...
      {auth.status === 'LIKELY_FAKE' && (
        <div style={{ margin: '0 12px 12px', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 10, padding: 10 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12, color: '#7F1D1D', lineHeight: 1.8 }}>
            ...
            <li>Return to chemist and ask for CDSCO registration proof</li>
            <li>Report: CDSCO helpline <strong>1800-180-3024</strong> (free)</li>
          </ul>
        </div>
      )}

      <button onClick={() => setExpanded(!expanded)} style={{ width: '100%', background: 'transparent', color: C.gray500, fontSize: 12, padding: '8px 14px', textAlign: 'left', borderTop: `1px solid ${cfg.border}` }}>
      ...
      {expanded && (
        <div style={{ padding: '0 14px 14px', fontSize: 12, color: C.gray700, lineHeight: 1.6 }}>
          ...
          <p style={{ margin: 0, color: C.gray500, fontStyle: 'italic' }}>
            Note: Agada uses AI visual analysis only. It cannot do a live CDSCO query in beta. Full integration coming soon.
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ background: C.blueLight, border: '2px solid #BFDBFE', borderRadius: 18, marginBottom: 12, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '0 14px 12px' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, background: info.prescriptionRequired ? '#FEE2E2' : '#DCFCE7', color: info.prescriptionRequired ? C.red : C.green, padding: '5px 12px', borderRadius: 20 }}>
          {info.prescriptionRequired ? '🩺 Prescription Required' : '✅ Available Over the Counter'}
        </span>
      </div>

      <button onClick={() => setExpanded(!expanded)} style={{ width: '100%', background: 'transparent', color: C.gray500, fontSize: 12, padding: '8px 14px', textAlign: 'left', borderTop: '1px solid #BFDBFE' }}>
        {expanded ? '▲ Hide warnings' : '▼ Show warnings & interactions'}
      </button>
  ...
  return (
    <div style={{ background: C.orangeLight, border: '2px solid #FED7AA', borderRadius: 18, marginBottom: 12, overflow: 'hidden' }}>
      <div style={{ padding: '14px 14px 10px' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: C.gray500, letterSpacing: '0.05em', textTransform: 'uppercase', marginBottom: 6 }}>
          ARE YOU OVERPAYING?
          <span style={{ marginLeft: 8, background: C.green, color: '#fff', borderRadius: 6, padding: '2px 7px', fontSize: 10 }}>Jan Aushadhi</span>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          <span style={{ fontSize: 24, lineHeight: 1 }}>💰</span>
        ...
        <div style={{ padding: '0 14px 12px' }}>
          <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid #FED7AA' }}>
            <div style={{ background: C.navy, color: '#fff', display: 'grid', gridTemplateColumns: '1fr auto auto', padding: '8px 12px', fontSize: 11, fontWeight: 700, gap: 8 }}>
              <span>Medicine</span><span style={{ textAlign: 'center' }}>Price</span><span style={{ textAlign: 'right' }}>Saves</span>
            </div>
            ...
            {brandedMrp && (
              <div style={{ background: '#FEF2F2', display: 'grid', gridTemplateColumns: '1fr auto auto', padding: '8px 12px', gap: 8, alignItems: 'center', borderBottom: '1px solid #FED7AA' }}>
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: C.gray700 }}>Branded (current)</div>
                </div>
            ...
            {alt.topAlternatives.map((med, i) => (
              <div key={i} style={{ background: i === 0 ? '#F0FDF4' : C.white, display: 'grid', gridTemplateColumns: '1fr auto auto', padding: '8px 12px', gap: 8, alignItems: 'center', borderBottom: i < alt.topAlternatives.length - 1 ? '1px solid #FED7AA' : 'none' }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 800, color: C.green, textAlign: 'center', whiteSpace: 'nowrap' }}>~₹{med.estimatedMrp}</span>
                <span style={{ fontSize: 11, fontWeight: 700, background: '#DCFCE7', color: C.green, borderRadius: 8, padding: '2px 6px', textAlign: 'right', whiteSpace: 'nowrap' }}>{med.savingsVsBranded}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ margin: '0 14px 14px', background: C.white, borderRadius: 10, padding: 10, border: '1px solid #FED7AA' }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: C.gray500, lineHeight: 1.6 }}>
          Yes. CDSCO requires all generics to be <strong>bioequivalent</strong> — same active ingredient, same effect. Price difference is branding, not medicine quality.
        </p>
      </div>
